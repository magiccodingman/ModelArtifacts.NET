...
public sealed class ArtifactManager : IDisposable
{
    ...
    public Task<ArtifactCandidate> RefreshAsync(CancellationToken cancellationToken = default) => ResolveCandidateAsync(true, cancellationToken);
    ...
    public async Task PromoteAsync(ArtifactCandidate candidate, bool cleanupObsoleteSnapshots = true, CancellationToken cancellationToken = default)
    {
        ValidateCandidate(candidate);
        if (!candidate.RequiresPromotion || candidate.CacheRoot is null) return;
        ...
        var snapshotsRoot = Path.Combine(candidate.CacheRoot, "snapshots");
        ...
        var currentPath = Path.Combine(candidate.CacheRoot, "current.json");
        ...
        if (cleanupObsoleteSnapshots)
            await DeleteOtherSnapshotsBestEffortAsync(candidate.CacheRoot, candidate.Snapshot.DirectoryPath, cancellationToken).ConfigureAwait(false);
    }
    ...
    {
        if (source.SourceKind == "local-directory") return;
        var cacheRoot = GetCacheRoot();
    ...
    private async Task<ArtifactCandidate> CreateLocalCandidateAsync(ResolvedArtifactSet resolved, CancellationToken cancellationToken)
    {
        ...
        var paths = resolved.Assets.Select(x => NormalizeAssetPath(x.Path)).Order(StringComparer.Ordinal).ToArray();
        var fingerprint = await ComputeFingerprintAsync(directory, paths, cancellationToken).ConfigureAwait(false);
        var snapshot = new ArtifactSnapshot(resolved.ArtifactSetId, source.SourceKind, source.SourceIdentity, resolved.Revision, fingerprint, directory, paths, false);
        return new ArtifactCandidate(snapshot, false, false, null, cacheIdentity);
    }

    private async Task<ArtifactSnapshot> DownloadCandidateAsync(string cacheRoot, ResolvedArtifactSet remote, CancellationToken cancellationToken)
    {
        var stagingRoot = Path.Combine(cacheRoot, "staging", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(stagingRoot);
        try
        {
            var normalizedAssets = remote.Assets.Select(asset => (Asset: asset, Relative: NormalizeAssetPath(asset.Path))).ToArray();
            ...
            var paths = normalizedAssets.Select(x => x.Relative.Replace('\\', '/')).Order(StringComparer.Ordinal).ToArray();
            ...
            var snapshotName = $"{ArtifactPathSafety.Sanitize(remote.Revision)}-{fingerprint[..12]}";
            var finalDirectory = Path.Combine(snapshotsRoot, snapshotName);
            if (Directory.Exists(finalDirectory))
                await DeleteDirectoryWithRetriesAsync(finalDirectory, cancellationToken, swallowFinalFailure: false).ConfigureAwait(false);
            Directory.Move(stagingRoot, finalDirectory);
            return new ArtifactSnapshot(remote.ArtifactSetId, source.SourceKind, source.SourceIdentity, remote.Revision, fingerprint, finalDirectory, paths, true);
        }
        catch
        {
            if (Directory.Exists(stagingRoot))
            {
                try { Directory.Delete(stagingRoot, true); } catch (IOException) { } catch (UnauthorizedAccessException) { }
            }
            throw;
        }
    }
    ...
    private async Task DownloadAssetAsync(ArtifactAsset asset, string destination, CancellationToken cancellationToken)
    {
        ...
        throw new ArtifactDownloadException($"Failed to download '{asset.Path}'.", lastError ?? new IOException("Download failed after retries."));
    }
    ...
    private async Task<ArtifactSnapshot?> TryLoadCurrentAsync(string cacheRoot, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (record is null || !string.Equals(record.CacheIdentity, cacheIdentity, StringComparison.Ordinal)) return null;
            var directory = Path.GetFullPath(Path.Combine(cacheRoot, "snapshots", record.DirectoryName));
            if (!Directory.Exists(directory)) return null;
            return new ArtifactSnapshot(record.ArtifactSetId, record.SourceKind, record.SourceIdentity, record.SourceRevision, record.Fingerprint, directory, record.AssetPaths, true);
        }
    ...
    private async Task<FileStream> AcquireLockAsync(string cacheRoot, CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ...
            catch (IOException) { await DelayAsync(options.LockRetryDelay, cancellationToken).ConfigureAwait(false); }
        }
    }
    ...
    private async Task DeleteOtherSnapshotsBestEffortAsync(string cacheRoot, string activeDirectory, CancellationToken cancellationToken)
    {
        ...
        {
            if (PathsEqual(directory, activeDirectory)) continue;
            await DeleteDirectoryWithRetriesAsync(directory, cancellationToken, swallowFinalFailure: true).ConfigureAwait(false);
        }
    }

    private async Task DeleteDirectoryWithRetriesAsync(string directory, CancellationToken cancellationToken, bool swallowFinalFailure)
    {
        for (var attempt = 0; attempt <= options.LockedFileDeleteRetries; attempt++)
        {
            try
            {
                if (options.DeleteDirectoryOverride is not null) await options.DeleteDirectoryOverride(directory, cancellationToken).ConfigureAwait(false);
    ...
    private Task DelayAsync(TimeSpan delay, CancellationToken cancellationToken) => options.DelayOverride?.Invoke(delay, cancellationToken) ?? Task.Delay(delay, cancellationToken);
    ...
    {
        if (string.IsNullOrWhiteSpace(remote.ArtifactSetId)) throw new ArtifactSourceException("Resolved artifact set ID is empty.");
        if (string.IsNullOrWhiteSpace(remote.Revision)) throw new ArtifactSourceException("Resolved artifact revision is empty.");
        if (remote.Assets.Count == 0) throw new ArtifactSourceException("Resolved artifact set contains no assets.");
        var paths = new HashSet<string>(StringComparer.Ordinal);
        foreach (var asset in remote.Assets)
        {
            var normalized = NormalizeAssetPath(asset.Path).Replace('\\', '/');
            if (!paths.Add(normalized)) throw new ArtifactSourceException($"Resolved artifact set contains duplicate path '{asset.Path}'.");
        }
    }
    ...
    private static string EnsureContained(string root, string relative)
    {
        ...
        if (!destination.StartsWith(rootFull + Path.DirectorySeparatorChar, comparison))
            throw new ArtifactDownloadException($"Artifact path '{relative}' escapes the staging directory.");
        return destination;
    }
    ...
    {
        var comparison = OperatingSystem.IsWindows() ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal;
        return string.Equals(Path.GetFullPath(left).TrimEnd(Path.DirectorySeparatorChar), Path.GetFullPath(right).TrimEnd(Path.DirectorySeparatorChar), comparison);
    }

    private static async Task<string> ComputeFingerprintAsync(string directory, IReadOnlyList<string> assetPaths, CancellationToken cancellationToken)
    {
        using var aggregate = IncrementalHash.CreateHash(HashAlgorithmName.SHA256);
        foreach (var relative in assetPaths.Select(x => x.Replace('\\', '/')).Order(StringComparer.Ordinal))
        {
